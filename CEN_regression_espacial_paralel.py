...
def main(config):
    ...
    for alias, alias_list in meta.items():
        ...
        MLR = spatial_MLR()
        MLR.regression_data(target_wind_filtrado,pd.DataFrame(ts_dict).apply(stand,axis=0),pd.DataFrame(ts_dict).keys().insert(0,'clim'))
        ...
        #            print('no anda i: ',i)
        #            print('no anda j: ',j)
        #MLR.perform_regression(config["work_dir"]+'/u850_1900_2099/'+alias)
        #Plot coefficients
